 ...
 *   #
 *   # Gotchas learned in @goyamegh/agent-health@0.5.21:
 *   #   - pass -m (else it uses the demo-model mock);
 *   #   - the judge alias must resolve to a valid Bedrock id WITHOUT -v1
 *   #     (us.anthropic.claude-opus-4-8), and the evaluator's
 *   #     inferenceConfig.temperature must be 1 for Opus 4.8 (it rejects 0.1);
 ...
 *   npx @goyamegh/agent-health@0.5.21 benchmark \
 *     -f evals/cc-vs-pi-redundant-apis.eval.js \
 *     -a claude-code -m claude-sonnet-4.6 \
 *     --evaluator api-redundancy-correctness --judge-model claude-opus-4.8 \
 *     --stop-server -v
 *   # then repeat with: -a pi -m amazon-bedrock/anthropic.claude-sonnet-4-6
 *
 *   # View results in an isolated UI:  AH_PORT=4191 npx @goyamegh/agent-health@0.5.21 serve -p 4191
 */

const { test, expect } = require('@goyamegh/agent-health');
...
// Raw inference-profile id (NOT the `claude-opus-4.8` alias): published
// @goyamegh/agent-health still maps that alias to the non-existent
// `us.anthropic.claude-opus-4-8-v1` profile (PR #347 fixed the alias but isn't
// published). The server resolves an unknown model string straight to Bedrock
// (`modelConfig?.model_id || modelKey`), so the raw id makes the Opus-4.8 judge
// actually run instead of erroring.
const JUDGE_MODEL = 'us.anthropic.claude-opus-4-8';
...
//   AH_PORT=4191 npx @goyamegh/agent-health@0.5.21 serve -p 4191 --headless &
