...
// ============================================================================
// OpenAI-compatible Fallback (Non-streaming)
// ============================================================================

/**
 * Get response from OpenAI-compatible endpoint.
 * Used as fallback when claude CLI is not available and provider is openai-compatible.
 */
async function streamFromOpenAICompatible(
  ...
  try {
    ...
    debug('Assistant', 'Using OpenAI-compatible model:', modelId, 'endpoint:', serverConfig.OPENAI_COMPATIBLE_ENDPOINT);
    ...
    if (!res.ok) {
      ...
      onError(`OpenAI-compatible endpoint responded ${res.status}: ${errorText}`);
      return;
    }
    ...
    const msg = error.message || 'Unknown OpenAI-compatible error';
    if (msg.includes('ECONNREFUSED') || msg.includes('ENOTFOUND')) {
      onError(`Cannot connect to OpenAI-compatible endpoint (${serverConfig.OPENAI_COMPATIBLE_ENDPOINT}). Ensure the server is running.`);
    } else {
      onError(msg);
    }
  }
