 ...
 * the wire payload to the UI for every judge call. In dev (`NODE_ENV !== production`)
 * the toggle defaults to ON so local iteration loops don't have to flip it.
 */
...
export function isJudgeDebugEnabled(): boolean {
  ...
  // Default on in dev/test for fast iteration; off in prod to keep run docs lean.
