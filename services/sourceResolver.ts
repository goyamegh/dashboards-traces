...
export async function resolveCodeFnMapForStoredTestCases(
  ...
  for (const [key, sourceFile] of filesByKey) {
    try {
      ...
      // stored; `lookup.resolved.root` records which root supplied it. A miss
      // is a load error like any other and surfaces through the hard
      // UnresolvableSourceFilesError below.
      const lookup = lookupSourceFile(sourceFile);
      if (!lookup.resolved) {
      ...
      // the SAME normaliser, relative to the eval root that supplied the file
