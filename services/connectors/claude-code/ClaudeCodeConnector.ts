...
export class ClaudeCodeConnector extends SubprocessConnector {
  ...
  override async execute(
    ...
    // appended config args). `super.execute()` reads `config.args`
    // synchronously before its first await, so this set→read window is
    // atomic in single-threaded JS and safe under benchmark concurrency.
    if (ccConfig) {
