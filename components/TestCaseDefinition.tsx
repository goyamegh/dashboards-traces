 ...
 *
 * NOTE ON PROVENANCE: this file is normally introduced by
 * opensearch-project/agent-health#420 ("Readable test-case definitions in
 * run views"). It is pre-added here, on goyamegh/testcase-context-pretty,
 * SOLELY so the composition with this branch's context-value pretty-printing
 * fix (lib/contextFormat.ts + components/evals3/ContextValueView.tsx) is
 * captured as a real, `git rerere`-trackable add/add conflict instead of a
 * silent one-sided file add that a from-scratch `main-goyamegh` rebuild
 * would otherwise drop every time (this file has no conflicting counterpart
 * to hook a resolution onto if only #420 ever introduces it). When #420
 * lands upstream, its copy of this file will replace this one; re-apply the
 * `ContextValueView` composition below (or drop this file from this branch
 * once #420 has merged and this fix's own PR rebases past it).
 */
