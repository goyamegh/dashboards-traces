...
### Changed
- **Test Case detail page — "Run Test" no longer hangs the page behind a blocking modal** ([components/evals3/TestCaseDetailPage.tsx](components/evals3/TestCaseDetailPage.tsx)): clicking *Run Test* used to open `QuickRunModal` over the whole page; the modal locked body scroll, disabled its own close button while running, and kept the user blocked for the full duration of the agent run with no way to look at the test case definition, switch tabs, or browse other runs. The fix mirrors the BenchmarkRunsPage UX so test-case runs feel identical to benchmark runs:
  - Clicking *Run Test* now opens a small **Configure Run** dialog (run name, description, agent, judge model, evaluator) that closes immediately on *Start Run*. The actual evaluation runs **inline** on the detail page — no full-screen modal, no body-scroll lock.
  - The header *Run Test* button morphs into a **Running… pill** (animated spinner, disabled while in flight) exactly like BenchmarkRunsPage's *Running…* state. *Edit* is also disabled during a run to prevent test-case mutation mid-evaluation.
  - A synthetic **RUNNING / Live** entry is pinned to the top of the *Test Case Runs* list (in both split-pane and full-width layouts) and auto-selected when the run starts, so the user can click it and watch the live trajectory without taking any extra action. The pill shows the streamed step count in real time.
  - The right inspector panel renders a new `LiveRunPanel` while the run is in progress: compact header with `RUNNING` badge + run name + judge model + step count, the original prompt for context, and a streaming `TrajectoryView` that auto-scrolls to the latest tool call as new AG-UI events arrive. The familiar reconnect banner from `QuickRunModal` is preserved verbatim so disconnect/recovery messaging stays consistent across entry points.
  - On completion, the runs list reloads and selection switches from the synthetic `__running__` ID to the real saved `reportId`, seamlessly transitioning the right panel into the normal `TestCaseInspectorPanel` (Summary / Conversation / Traces / LLM Judge / Annotations tabs) without a flash or navigation.
  - Wiring uses the same `runServerEvaluation` SSE path the modal used (`onStep` accumulates into a local `liveSteps` array, `onReconnect` / `onPoll` drive the banner). Agent / model selections persist to the same `prefs:agentKey` / `prefs:modelId` localStorage keys QuickRunModal and BenchmarkRunsPage already share, so the choice carries across pages. `QuickRunModal` itself remains in the codebase — it's still used for ad-hoc Quick Run from the Test Cases list — but is no longer reachable from the detail page.

### Added
- **Evaluator version history with Git-style diff viewer** ([components/diff/UnifiedDiffView.tsx](components/diff/UnifiedDiffView.tsx), [components/evaluators/EvaluatorVersionHistory.tsx](components/evaluators/EvaluatorVersionHistory.tsx), [components/EvaluatorEditPage.tsx](components/EvaluatorEditPage.tsx)): every save of a custom evaluator already produced an immutable `EvaluatorVersion` snapshot (system prompt + scoring config + inference config), and the backend already exposed `GET /api/storage/evaluators/:id/versions`, but there was no UI to browse those snapshots — users had no way to see what changed between v3 and v4 of a judge prompt. This mirrors the History tab on the Skills page (which lists past iterations) and adds the Git-style diff that was missing there too. Three pieces:
  - **`UnifiedDiffView`** — a dependency-free, reusable Git-style diff component. Implements line-level diff via a Longest Common Subsequence walk over the two inputs (≈80 LOC, no `diff`/`react-diff-viewer` dependency added) and renders the result in either *unified* mode (single column, `+`/`-` prefixes, red/green tinted rows) or *split* mode (two columns, before vs after aligned per hunk). Equal regions outside `contextLines` of any change collapse into a `@@ N unchanged lines hidden` gutter, exactly like `git diff --unified=N`. The header shows `+X / −Y` counts; identical inputs render an explicit "No differences" message instead of an empty pane.
  - **`EvaluatorVersionHistory`** — a Card that lists every version of the evaluator (newest first, `latest` badge on v_max), with a checkbox per row capped at two selections. The `Compare (N/2)` button is disabled until two are picked; clicking it opens a dialog that diffs *all three* content fields (system prompt, scoring config JSON, inference config JSON) using `UnifiedDiffView`, with a Unified/Split toggle. An eye icon on each row opens a separate read-only modal showing that version's full content. System evaluators (immutable v1) and brand-new evaluators (still v1) render an explanatory empty state instead of an empty table.
  - **Wiring in `EvaluatorEditPage`** — the history Card is mounted between Scoring Configuration and Inference Configuration, but only when `isEditMode && evaluatorId` (so the New Evaluator form stays clean). Saving a new version no longer auto-navigates back to the list when in edit mode; instead it bumps a `historyRefresh` counter so the user can immediately see the freshly-minted version land in the history table below. Creating a brand-new evaluator still navigates to `/evaluators` as before.
  - **Tests** (8 new, [tests/unit/components/diff/UnifiedDiffView.test.ts](tests/unit/components/diff/UnifiedDiffView.test.ts)): `diffLines` returns all-equal ops on identical input with line numbers in lockstep, records pure-additions / pure-removals correctly, represents replacements as remove+add bookended by equal lines, handles empty `before`/`after` without crashing, and preserves the LCS across interleaved changes (`a/B/c/D/e` ↔ `a/b/c/d/e` produces 3 unchanged + 2 add + 2 remove with `a, c, e` as the equal anchors). `summarizeDiff` counts adds, removes, and unchanged lines independently.
...
- **AI assistant chat now runs as a real Claude Code session with full tool access by default** (`server/services/assistantService.ts`, `hooks/useAssistantRuntime.ts`, `types/index.ts`): previously the assistant had Claude CLI under the hood but was deliberately neutered — it spawned with `--disallowed-tools '*'` and a system prompt that explicitly said "NO tools enabled". The assistant could only answer from a small inlined snapshot, which is why questions like "which tests passed for which agent?" on the comparison page came back empty (the comparison route's URL parser also wasn't picking up `benchmarkId`, so even the snapshot was empty there). Three changes, all on by default — no flag:
  ...
  - **System prompt rewritten with two variants.** `buildSystemPrompt(context, toolsAvailable)` now branches: when `toolsAvailable=true` (Claude CLI path), the prompt advertises the available tools, lists the local Agent Health endpoints (`http://localhost:${AGENT_HEALTH_PORT||4001}/api/storage/runs/:id`, `…/runs/by-benchmark-run/:bench/:run`, `…/benchmarks/:id`, etc.), explains the hash-router URL shape (`http://localhost:${AGENT_HEALTH_DEV_PORT||4000}/#/compare/<bench>?runs=<a>,<b>`), and gives a concrete "how to answer 'which tests passed for which agent?'" workflow that combines the comparison snapshot with `curl` against the backend. When `toolsAvailable=false` (Bedrock / LiteLLM fallbacks), the original "no tools — use only the snapshot" policy stays in place. The tool-use NDJSON branch in `streamFromClaude` no longer prints "tool execution is disabled" — it now renders a friendly inline `_🔧 Using \\\`tool_name\\\`…_` marker while the tool runs, and tool_result events are suppressed from the UI (the model summarizes them in its next text block).
