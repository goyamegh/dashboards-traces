...
- **Regression coverage — the tests that didn't catch any of this**:
  ...
  - `tests/e2e/evals3-benchmarks.spec.ts` had **no test for an Edit button, no version-badge assertion, and no test that mutating test cases produces v2**. Added a `Evals3 Benchmarks Page — version badge & row Edit` describe pinning three contracts: every row renders `data-testid="benchmark-version-badge"` reading `v{n}` (defaults to v1); every row exposes `data-testid="edit-benchmark-button-row"`; clicking it opens the editor in *edit* mode (asserts "Edit Benchmark" header is present, "Create Benchmark" is absent).
  - `tests/e2e/evals3-benchmark-runs.spec.ts` previously only asserted that the Tabs were visible — trivially true under "Option B" but not the legacy view users actually wanted. Added:
    - `Evals3 Benchmark Runs Page — Edit & versioning` (4 tests): header has exactly one `data-testid="edit-benchmark-button"`, header shows the current `v{n}` badge, clicking Edit opens the editor in *edit* mode, and a full E2E walking the wizard, changing the test-case selection, capturing the PUT/PATCH update response, polling the storage API until `currentVersion === 2`, asserting both seeded test-case IDs are present, and asserting the in-place header badge flips to `v2` without a navigation.
    - `Evals3 Benchmark Runs Page — Edit without forced run` (2 tests): Step 2 with test-case changes exposes both `[data-testid="editor-save-without-run"]` and `"Next: Define Runs"`; clicking the save-without-run button bumps `currentVersion` server-side, the editor closes, **no `POST /execute` request is fired** (asserted by hooking `page.on('request', ...)`), and `runs[].length` does not grow.
    - 2 layout-default tests: split layout is the default after wiping `localStorage` (Test Cases + Runs panel headers visible side-by-side, no `[role="tablist"]`, layout toggle `aria-pressed="true"` on the Split side); toggling to Tabs persists across reload.
  - `tests/unit/server/routes/storage/benchmarks.test.ts` had no version-bump coverage for `PUT /api/storage/benchmarks/:id`. Added 4 unit tests against the route handler: bumps when `testCaseIds` change (asserts `currentVersion === 2` AND `versions[]` grew with the new entry); does NOT bump when only metadata changes (`name` / `description`); treats reordered IDs as no change (set semantics, protecting against accidental version inflation from UI re-renders); and bumps when removing a test case (length shrinks).
  - `tests/integration/server/routes/storage/benchmarks.integration.test.ts` had no Edit/versioning section at all. Added `Benchmark Edit & Versioning Integration Tests` with 5 real-HTTP tests against a live backend: PUT with new IDs bumps v1→v2 (with GET round-trip proving persistence); metadata-only does NOT bump; reordered IDs do NOT bump; removing a TC bumps with the shorter list; and a chain test that walks v1→v2→v3→v4 across successive PUTs verifying earlier `versions[]` entries remain immutable.
