...
### Changed
- **Comparison page rebuilt as a diagnosis surface** ([components/ComparisonPage.tsx](components/ComparisonPage.tsx), [services/comparisonService.ts](services/comparisonService.ts), [services/trajectoryDiffService.ts](services/trajectoryDiffService.ts), [services/failureClusterService.ts](services/failureClusterService.ts), [server/routes/comparison.ts](server/routes/comparison.ts)): the page used to be two side-by-side scoreboards plus a radar — the user had to read deltas and expand every row to see what changed. Five-phase rework:
...
- **`SubprocessConnector` — three streaming / configuration bugs that were blocking clean Kiro runs through the UI** ([services/connectors/subprocess/SubprocessConnector.ts](services/connectors/subprocess/SubprocessConnector.ts)):
  ...
  - **Shell-quote prompt input when `inputMode='arg'`.** `spawn(..., { shell: true })` re-joins args via `/bin/sh`, so unquoted spaces or slashes in the prompt got word-split — a prompt like `"/cp-oncall investigate <url>"` arrived at the child as 3 separate args, breaking CLIs that interpret the first unrecognized token as a subcommand.
