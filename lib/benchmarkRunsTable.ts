...
export type RunSortField =
  | 'name' | 'agent' | 'model' | 'size' | 'concurrency' | 'passRate' | 'judge' | 'evaluator' | 'date'
...
export function toggleRunSort(current: RunSort, field: RunSortField): RunSort {
  ...
  const numeric: RunSortField[] = ['size', 'concurrency', 'passRate', 'date', 'tokens', 'cost', 'llmCalls', 'timePerCase'];
