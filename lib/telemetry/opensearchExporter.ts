...
/**
 * Custom OpenSearch Span Exporter
 *
...
export interface OpenSearchExporterConfig {
  ...
  /** Basic auth password */
  password?: string;
  /** Index pattern to write to (default: otel-v1-apm-span-000001) */
  indexName?: string;
...
/**
 * Convert OTel HrTime [seconds, nanoseconds] to ISO date string
 */
...
/**
 * Convert OTel HrTime to nanoseconds
 */
...
/**
 * Convert duration between two HrTimes to nanoseconds
 */
...
/**
 * Map OTel SpanStatusCode to OSIS integer
 */
...
/**
 * Convert a ReadableSpan to an OSIS-compatible OpenSearch document
 */
function spanToDocument(span: ReadableSpan): Record<string, unknown> {
  ...
  // traceGroupFields (populated for root spans)
...
/**
 * Custom SpanExporter that writes directly to OpenSearch.
 *
 * Formats spans in OSIS-compatible format so eval spans co-exist
