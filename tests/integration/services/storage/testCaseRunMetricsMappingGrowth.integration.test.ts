 ...
 *      mapping — only dropping the index does). *   2. Test A synthesizes ONE report with 1000+ distinct custom metric names
...
const REAL_INDEX = STORAGE_INDEXES.runs;// > 1000 distinct nested matcher/attr keys, per the PR requirement — split
...
describeIfOptedIn('TestCaseRun metrics/judgeMetrics mapping growth (real OpenSearch)', () => {
  ...
  beforeAll(async () => {
    ...
    available = await rawClusterReachable(client);    if (!available) {
      // eslint-disable-next-line no-console
  ...
  it('persists a report with 1000+ distinct custom metric/judgeMetrics names and does not grow the mapping', async () => {
    ...
    const reportId = uniqueTestName('report-mapping-growth');    const metricPrefix = `custom_metric_${RUN_TAG}`;
    ...
    const report: Partial<TestCaseRun> = {
      id: reportId,
      testCaseId: `ahtest-tc-${RUN_TAG}`,      agentName: 'demo-agent',
      modelName: 'test-model',
      status: 'completed',
      passFailStatus: 'passed',
      trajectory: [],
    ...
    const afterFieldCount = countMappedFields((afterMapping.body as any)[index].mappings.properties);    expect(afterFieldCount - beforeFieldCount).toBeLessThanOrEqual(FIELD_COUNT_SLACK);
    ...
    // assertion.
    const metricsMapping = (afterMapping.body as any)[index].mappings.properties.metrics;    expect(String(metricsMapping.dynamic)).toBe('false');
    ...
    const judgeMetricsMapping = (afterMapping.body as any)[index].mappings.properties.matcherResults.properties      .judgeMetrics;
  ...
  it('keeps every field OpenSearchRunOperations.search() filters/sorts on real and queryable', async () => {
    ...
    await storage.runs.create({
      id: reportId,
      testCaseId: `ahtest-query-audit-tc-${RUN_TAG}`,      agentName: 'demo-agent',
      modelName: 'test-model',
      status: 'completed',
      passFailStatus: 'passed',
      trajectory: [],
    ...
    const { items } = await storage.runs.search({ testCaseId: `ahtest-query-audit-tc-${RUN_TAG}` });    expect(items.map((r) => r.id)).toContain(reportId);
  }, 30000);
});
