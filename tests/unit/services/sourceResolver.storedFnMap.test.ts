...
// "exists" and resolves against cwd exactly like the real single-root
// default, minus the fs check); the multi-root behaviour has its own suite
...
jest.mock('@/lib/config/evalRoots', () => {
  ...
  return {
    lookupSourceFile: jest.fn((f: string) => {
      ...
      const abs = p.isAbsolute(posix) ? p.normalize(posix) : p.resolve(cwd, posix);
      return { resolved: { abs, root: p.isAbsolute(posix) ? null : cwd }, tried: [cwd] };
    }),
    toStoredSourceFile: jest.fn((abs: string) => ({ sourceFile: abs, root: process.cwd() })),
