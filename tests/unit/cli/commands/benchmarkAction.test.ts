...
describe('Benchmark Command - Real Module Coverage', () => {
  ...
  describe('command action', () => {
    ...
    it('runs quick mode as an ad-hoc evaluation run — no benchmark doc is created', async () => {
      ...
      // Quick mode is ad-hoc (unified sources): it neither creates a benchmark
      // nor runs one through executeBenchmarkAsEvaluationRun (that path is for
      // -n <existing> / single JSON -f), and never the legacy /execute route.
