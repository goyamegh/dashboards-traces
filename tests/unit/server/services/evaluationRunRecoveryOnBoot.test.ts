...
describe('recoverOrphanEvaluationRunsSafely', () => {
  ...
  it('never throws even when storage explodes (fire-and-forget: does not return a rejecting promise)', async () => {
    const storage = {
      evaluationRuns: { list: jest.fn().mockRejectedValue(new Error('boom')) } as any,
    } as unknown as IStorageModule;

    // Synchronous, fire-and-forget wrapper (returns void, not a Promise) —
    // calling it must not throw synchronously, and the async work it kicks
    // off must not produce an unhandled rejection.
    expect(() => recoverOrphanEvaluationRunsSafely(storage)).not.toThrow();
  ...
  it('logs a summary line', async () => {
    ...
    // Fire-and-forget: give the internal .then() chain a tick to run.
    await new Promise((resolve) => setImmediate(resolve));
    await new Promise((resolve) => setImmediate(resolve));
